...
export const BookItem = ({id, title, bookImage, authors, categories}: IBookItem) => {
    return (
        <div className="border border-b-gray-200 rounded relative group w-full">
            <div className="m-2 rounded relative bg-gray-50 p-2">
                <img className="align-middle rounded object-cover" src={bookImage} />
            ...
            <div className="text-center p-3">
                <h4><Link to={"/"} className="text-black hover:text-blue-600 title-cart">{title}</Link></h4>
                <div>
                    <h6 className="text-sky-500">{authors.map(author => author.fullName).join(", ")}</h6>
                </div>
                <div className="text-blue-600">{categories.map(category => category.name).join(", ")}</div>
            </div>
        </div>
    )
}