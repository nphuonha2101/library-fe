...
export const BookManagement: React.FC = () => {
    ...
    // Xử lý nút Sửa trong DataTable
    const handleEdit = (row: IBookItem) => {
        setSelectedRow(row);
        setEditModalOpen(true);
    };

    // Xử lý nút Xóa trong DataTable
    const handleDelete = async (row: IBookItem) => {
        const token = getStoredToken() || await getAntiForgeryToken();
        fetch(`https://localhost:7259/api/v1/books/${row.id}`, {
            method: 'DELETE',
            headers: {
    ...
    // Gọi API để sửa sách khi nhấn nút Sửa trong Modal
