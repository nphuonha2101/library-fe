...
export const BookManagement: React.FC = () => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`https://localhost:7259/api/v1/books/${selectedRow?.id}`, {
                method: 'POST',
                headers: {
                ...
                <Modal.Body className="overflow-y-auto">
                    <form id="editBookForm" className="w-full" onSubmit={editBook}>
                        <div className="grid gap-2 mb-3 grid-cols-1 md:grid-cols-2">
                            ...
                            <div>
                                <label htmlFor="quantity" className="block mb-1 text-sm font-medium text-gray-900 dark:text-white">
                                    Số lượng
                                </label>
                                <input
                                    type="number"
                            ...
                            <div>
                                <label htmlFor="isbn" className="block mb-1 text-sm font-medium text-gray-900 dark:text-white">
                                    ISBN
                                </label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label htmlFor="authors" className="block mb-1 text-sm font-medium text-gray-900 dark:text-white">
                                    Tác giả
                                </label>
                                <select
                                    id="authors"
                                ...
                                >
                                    {authors && authors.map((author) => {
                                        if (selectedRow?.authors?.find(auth => auth.id === author.id))
                                            return <option key={author.id} value={author.id} selected>{author.fullName}</option>
                                        else
                            ...
                            <div className="col-span-2">
                                ...
                                <SlateEditor onSave={handleEditorSave} handleSubmit={handleSubmit} />
                            </div>
                            <div className="flex items-center justify-center w-full col-span-2">
                                <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                        ...
                                        <p className="mb-2 text-sm text-gray-500 dark:text-gray-400"><span className="font-semibold">Chọn để tải lên</span> hoặc kéo thả file vào đây</p>
                                        <p className="text-xs text-gray-500 dark:text-gray-400">JPG, JPEG, PNG, WEBP</p>
                                    </div>
                                    <input id="dropzone-file" type="file" name="file-upload" accept="image/png, image/jpg, image/jpeg, image/webp" className="hidden" onChange={handleFileChange} />
                                </label>
                            </div>
                            <div className="col-span-2 mt-2">
                                {selectedFile && (
                                    <div>
                                        <h3 className="text-sm font-medium text-gray-900 dark:text-white">Selected File đã chọn :</h3>
                                        <p>{selectedFile.name}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        <button type="submit" className="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                            <svg className="me-1 w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
